...
video_list = 'videos.txt' # a list of paths
...
for index, file in zip(videos['index'], videos['file']):
    ...
    for i in itertools.count():
        ...
        time_image = frame[time_bounds.y1:time_bounds.y2, time_bounds.x1:time_bounds.x2]
        time = pytesseract.image_to_string(time_image, config=time_config).strip()

        print()
        org = cv2.cvtColor(org, cv2.COLOR_RGB2BGR)
        print(f'{time} {level} {lives}')
        rect_color = (255, 135, 70)
        thickness = 2
        text_rect_color = (51, 51, 255)
        text_thickness = 1
        cv2.rectangle(org, (yellow_bounds.x1, yellow_bounds.y1), (yellow_bounds.x2, yellow_bounds.y2), rect_color, thickness)
        cv2.rectangle(org, (black_bounds.x1, black_bounds.y1), (black_bounds.x2, black_bounds.y2), rect_color, thickness)
        cv2.rectangle(org, (lives_white_bounds.x1, lives_white_bounds.y1), (lives_white_bounds.x2, lives_white_bounds.y2), rect_color, thickness)
        cv2.rectangle(org, (lives_bounds.x1, lives_bounds.y1), (lives_bounds.x2, lives_bounds.y2), text_rect_color, text_thickness)
        cv2.rectangle(org, (level_bounds.x1, level_bounds.y1), (level_bounds.x2, level_bounds.y2), text_rect_color, text_thickness)
        cv2.imshow('display', org)
        key = cv2.waitKey(0)
        if key == ord('s'):
            cv2.imwrite('frame.png', org)
